...
This repository should enforce branch protection on `main` (and `master`
if still used) so pull requests cannot merge unless CI passes.
...
5.  Enable **Dismiss stale pull request approvals when new commits are
    pushed**.
...
If check names vary by matrix label, require each visible
`R-CMD-check (...)` entry.

## Why this matters

This ensures failures are caught on PR push/update rather than after
merge to `main`, including pkgdown index drift and documentation
consistency issues.

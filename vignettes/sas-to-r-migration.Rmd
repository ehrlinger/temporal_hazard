---
title: "Migrating from SAS HAZARD to hvtiRhazard"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Migrating from SAS HAZARD to hvtiRhazard}
...
HAZARD analysis into native R using `hvtiRhazard`.
...
- `hvtiRhazard` coerces `status` to `numeric`; logical vectors are also accepted.
...
In `hvtiRhazard` these are unified into a single design matrix `x` and
...
Stepwise variable selection is **not yet implemented** in `hvtiRhazard`. Planned
...
`hvtiRhazard` is currently pure R. If profiling against large real datasets

...
```{mermaid}
%%| label: fig-callgraph
%%| fig-cap: "Call graph for a multiphase hazard fit"
flowchart TD
    A["hazard(dist='multiphase')"] --> B[".hzr_optim_multiphase()"]
    B --> C["Resolve per-phase design matrices"]
    B --> D[".hzr_phase_start() per phase"]
    B --> E[".hzr_optim_generic()"]
    E --> F["stats::optim(method='BFGS')"]
    F --> G[".hzr_logl_multiphase()"]
    G --> H[".hzr_split_theta()"]
    G --> I[".hzr_multiphase_cumhaz()"]
    G --> J[".hzr_multiphase_hazard()"]
    I --> K["hzr_phase_cumhaz()"]
    J --> L["hzr_phase_hazard()"]
    K --> M["hzr_decompos()"]
    L --> M
    A --> N["predict.hazard()"]
    N --> I
...
theta_layout <- data.frame(
  `Phase type` = c("cdf / hazard", "constant"),
  `Sub-vector` = c(
...
knitr::kable(datasets, caption = "Reference datasets in inst/extdata/")
```
...
The SAS late phase uses TAU, GAMMA, ALPHA, ETA:

- **TAU** → `t_half` (late-phase characteristic time)
- **ALPHA** → `m` (shape exponent)
- **ETA** → `nu` (time exponent)
- **GAMMA** — Jointly determines nu and m in the decompos family;
  collapsed into the three-parameter representation. When the Weibull
  flag is active (the common case for late phases), GAMMA controls the
  power of the late-phase hazard rise.

...
candidate pool, and let the algorithm decide.  The scope is passed as
a flat character vector (applied to every phase in the multiphase
model); stepping operates per `(variable, phase)` pair so a covariate
can enter one phase and not another.
